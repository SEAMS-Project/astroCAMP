#!/usr/bin/env python3
"""
Plot 34c: stacked problem size with IDG GB/J on a secondary y-axis for 8k and 32k workloads.
"""
...
def main() -> int:
    size_df = (
        ...
        .sort_values(["Image size", "Timesteps", "Channels"])
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"c={int(v)}" for v in merged["Channels"]], rotation=90, fontsize=10)

    t_positions = []
    t_labels = []
    for (_, timesteps), group in merged.groupby(["Image size", "Timesteps"], sort=True):
        t_positions.append(x[group.index].mean())
        t_labels.append(f"t={int(timesteps)}")

    t_groups = list(merged.groupby(["Image size", "Timesteps"], sort=True))
    for idx in range(1, len(t_groups)):
        prev_key, prev_group = t_groups[idx - 1]
        curr_key, curr_group = t_groups[idx]
        # Only separate timestep blocks within the same image-size section.
